...
public class PatternSimilarity extends SimilarityFunction{
	
	private static Logger logger = Logger.getLogger(PatternSimilarity.class);
	private String namefunction;

		
	public PatternSimilarity() {
		this.namefunction="PATTERN_SIMILARITY";
	}
	...
	public  double getSimilarity(UserModel u1 , UserModel u2) {
		...
		for( String conceptid : concept2singnal.keySet() ){
			...
			if(signals[0]!=null && signals[1]!=null){
				...
				for(int i=0;i<sign1.length;i++){
					...
					double compEnergy=energy1+energy2;
					tot+= compEnergy*phaseSimilarity;
		...
		double similarity=2*tot/globalEnergy;
		return    similarity;
	}
